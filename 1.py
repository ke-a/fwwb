...
import pandas as pd
# 读取微型气象站和标准站的数据
mini_weather = pd.read_csv('micro2.csv')  # 读取微型气象站数据
...
def correct_data(row):
    ...
    # 如果NO2数据误差大于20或为NaN，则将其修正为标准站数据
...
X = scaler.fit_transform(
    mini_weather[['PM2.5', 'PM10', 'NO2', 'temperature', 'humidity']])
...
regressor = MLPRegressor(hidden_layer_sizes=(
    100, 100), activation='relu', solver='adam', max_iter=500)
