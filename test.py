...
# 对微型站数据进行预测和校准
X_micro_unlabeled = micro_data[['temperature', 'humidity', 'NO2']].fillna(0).values  # 提取微型站中与PM2.5相关的特征数据，并将缺失值填充为0
y_pred = model.predict(X_micro_unlabeled)  # 利用训练好的模型对微型站数据进行预测
lp_model = LabelPropagation()  # 创建标签传播模型
lp_model.fit(X_micro_unlabeled, y_pred.ravel().astype('int'))  # 利用未标记的数据对模型进行半监督训练和校准
y_calibrated = lp_model.predict(y_pred.reshape(-1, 1))  # 对预测结果进行校准
